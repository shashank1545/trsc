...
public:
  ...
  RetTy visit(Expr *E) {
    ...
    case ASTNodeKind::ASTK_STRINGEXPR:
      return getDerived().visitStringExpr(static_cast<StringExpr *>(E));
  ...
  RetTy visitStringExpr(StringExpr *E) { return RetTy(); }
  ...
  RetTy visitUnaryExpr(UnaryExpr *E) {
    getDerived().visit(E->getOperand());
    return RetTy();
  }
