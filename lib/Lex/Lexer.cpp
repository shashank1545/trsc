...
Token Lexer::Lex() {
  ...
  case '(':
    CurPtr++;
    return FormToken(TokenKind::DE_LPAREN, TokenStart);
  case ')':
    CurPtr++;
    return FormToken(TokenKind::DE_RPAREN, TokenStart);
  case '{':
    CurPtr++;
    return FormToken(TokenKind::DE_LBRACE, TokenStart);
  case '}':
    CurPtr++;
    return FormToken(TokenKind::DE_RBRACE, TokenStart);
  case '[':
    CurPtr++;
    return FormToken(TokenKind::DE_LBRACKET, TokenStart);
  case ']':
    CurPtr++;
    return FormToken(TokenKind::DE_RBRACKET, TokenStart);
  case ';':
    CurPtr++;
    return FormToken(TokenKind::DE_SEMICOLON, TokenStart);
  case ',':
    CurPtr++;
    return FormToken(TokenKind::DE_COMMA, TokenStart);
  case ':':
    if (*(CurPtr + 1) == ':') {
      CurPtr += 2;
  ...
  case '%':
    CurPtr++;
    return FormToken(TokenKind::OP_PERCENT, TokenStart);
  case '.':
  ...
  case '*':
    CurPtr++;
    return FormToken(TokenKind::OP_STAR, TokenStart);
  case '<':
    if (*(CurPtr + 1) == '=') {
      CurPtr += 2;
